...
exec export NODE_ENV=clouddev
exec /usr/bin/node /advantage/bin/www' > /etc/init/node-advantage.conf

echo "Checking Service configuration for /etc/init/node-advantage.conf"
sudo init-checkconf /etc/init/node-advantage.conf

echo "Starting the /etc/init/node-advantage"
sudo service node-advantage start

echo "Check the status of /etc/init/node-advantage"
